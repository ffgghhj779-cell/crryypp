  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.05] bg-[#0f0f0f]">
        <div>
          <p className="text-[9px] text-white/25 uppercase tracking-widest font-mono">GARCH(1,1) · Volatility Bands</p>
          <p className="text-white font-bold text-base font-mono tabular-nums">{symbol}</p>
        </div>
        <div className="flex flex-col items-end gap-1.5">
          {/* Regime badge */}
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg border ${stateColor}`}>
      ...
      <div className="mx-4 my-3 rounded-xl border border-white/[0.07] bg-[#0a0a0a] divide-y divide-white/[0.05]" dir="rtl">
        ...
        <div className="flex items-center justify-between px-3 py-3">
          <span className="text-[10px] text-white/40 leading-snug text-right">
            الحد العلوي المتوقع<br />
            <span className="text-white/20">(مقاومة)</span>
          </span>
          <div className="text-left">
            <p className="text-sm font-mono font-bold text-white tabular-nums">${fmtPrice(data.upperBound)}</p>
            <p className="text-[11px] font-mono font-bold text-orange-400 tabular-nums">
        ...
        <div className="flex items-center justify-between px-3 py-3 relative">
          ...
          <span className="text-[10px] text-white/40 text-right pr-4">سعر الإغلاق الحالي</span>
          <p className="text-base font-mono font-bold text-white tabular-nums">${fmtPrice(data.currentPrice)}</p>
        </div>
        ...
        <div className="flex items-center justify-between px-3 py-3">
          ...
          <div className="text-left">
            <p className="text-sm font-mono font-bold text-white tabular-nums">${fmtPrice(data.lowerBound)}</p>
            <p className="text-[11px] font-mono font-bold text-orange-400 tabular-nums">
      ...
      <div className="mx-4 mb-3 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={label} className="rounded-xl bg-white/[0.03] border border-white/[0.05] px-2 py-2 text-center">
            ...
            <p className="text-xs font-mono font-bold text-orange-300 tabular-nums">{value}</p>
          </div>
        ))}
      </div>
