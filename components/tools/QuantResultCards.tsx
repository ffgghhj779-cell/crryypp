    ...
    <Shell>
      ...
      <div className="mx-4 my-3 rounded-xl border border-white/[0.07] bg-[#0a0a0a] divide-y divide-white/[0.05]" dir="rtl">
        <div className="flex items-center justify-between px-3 py-4">
          <span className="text-sm text-white/40 leading-snug">السعر الأقصى المتوقع<br /><span className="text-white/20">(95th percentile)</span></span>
          <div className="text-left">
            <p className="text-base font-mono font-bold text-emerald-300 tabular-nums">${fmtPrice(data.projectedHigh)}</p>
        ...
        <div className="flex items-center justify-between px-3 py-4 relative">
          ...
          <span className="text-sm text-white/40 pr-4">السعر المتوقع (المتوسط)</span>
          <p className="text-lg font-mono font-bold text-white tabular-nums">${fmtPrice(data.expectedPrice)}</p>
        </div>
        <div className="flex items-center justify-between px-3 py-4">
          <span className="text-sm text-white/40 leading-snug">السعر الأدنى المتوقع<br /><span className="text-white/20">(5th percentile)</span></span>
          <div className="text-left">
            <p className="text-base font-mono font-bold text-red-300 tabular-nums">${fmtPrice(data.projectedLow)}</p>
      ...
      <div className="mx-4 mb-3 flex justify-between items-center" dir="rtl">
        <span className="text-sm text-white/25 font-mono">السعر الحالي</span>
        <span className="text-base font-mono font-bold text-orange-300 tabular-nums">${fmtPrice(data.currentPrice)}</span>
      </div>
...
export function LinearRegressionCard({ data, symbol, timeframe }: LRProps) {
  ...
  return (
    <Shell>
      ...
      {/* Slope chip */}
      <div className="mx-4 mb-3 flex justify-between items-center" dir="rtl">
        <span className="text-sm text-white/25 font-mono">الميل (Slope / شمعة)</span>
        <span className={`text-base font-mono font-bold tabular-nums ${data.isTrendUp ? 'text-emerald-400' : 'text-red-400'}`}>
  ...
  return (
    <Shell>
      ...
      <div className={`mx-4 my-3 rounded-xl border border-white/[0.06] bg-[#0a0a0a] px-5 py-6 text-center shadow-lg ${style.glow}`} dir="rtl">
        <p className="text-sm text-white/25 uppercase tracking-widest font-mono mb-3">النظام الحالي للسوق</p>
  ...
  return (
    <Shell>
      ...
      <div className="mx-4 mb-3" dir="rtl">
        <p className="text-sm text-white/25 font-mono mb-1.5 text-right">موضع الدورة الحالية</p>
