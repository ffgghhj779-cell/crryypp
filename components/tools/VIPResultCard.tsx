...
const inputCls =
  'w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] ' +
  'text-white font-mono text-sm focus:outline-none focus:border-orange-500/60 ' +
...
export function VIPResultCard({ symbol }: Props) {
  ...
  return (
    <div className="space-y-3" style={{ animation: 'fade-in 0.2s ease forwards' }}>
      ...
      {/* Input Form */}
      <div className="rounded-2xl border border-white/[0.07] bg-[#111111] p-4 space-y-3">
        <p className="text-[9px] text-white/25 uppercase tracking-widest font-mono">Trading VIP · إعداد التمركز</p>
        ...
        {/* Error */}
        {error && (
          <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl border border-red-500/30 bg-red-500/[0.07]">
        ...
        <button
          ...
          className="w-full py-3.5 rounded-xl font-bold text-sm tracking-wider transition-all active:scale-[0.98] disabled:cursor-not-allowed"
          style={{
      ...
      {phase === 'done' && result?.rejected && (
        <div
          className="rounded-2xl border border-white/[0.06] bg-[#121212] px-4 py-5 text-center space-y-2"
          ...
          <div className="grid grid-cols-5 gap-1.5 mt-4 pt-4 border-t border-white/[0.05]">
            {result.factors.map(f => (
              <div key={f.label} className="text-center">
                <p className="text-[10px] font-mono font-bold text-white/30 tabular-nums">{f.score}</p>
                <p className="text-[9px] text-white/20 mt-0.5">{f.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {phase === 'done' && result && !result.rejected && (() => {
        ...
        return (
          ...
          >
            {/* Header */}
            <div className="px-4 pt-4 pb-3 border-b border-white/[0.05] bg-[#0f0f0f]" dir="rtl">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[9px] text-white/25 uppercase tracking-widest font-mono mb-0.5">Trading VIP · إجماع متعدد الأطر</p>
                  <p className="text-white font-bold text-sm">سوينج VIP ممتد</p>
                </div>
                <div className="text-right">
                  <p className="text-4xl font-black text-orange-500 tabular-nums font-mono leading-none">{score}%</p>
                  <span className="text-[9px] text-white/30 font-mono border border-white/[0.1] px-2 py-0.5 rounded mt-1 inline-block">
                    TIMEFRAME: 4H
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-5 divide-x divide-white/[0.05] border-b border-white/[0.05]" dir="rtl">
              {[...factors].reverse().map(f => (
                <div key={f.label} className="py-3 text-center">
                  <p className="text-[11px] font-mono font-bold text-orange-400 tabular-nums">{f.score}%</p>
                  <p className="text-[9px] text-white/30 mt-0.5">{f.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="px-4 py-3 border-b border-white/[0.05] text-center">
              <p className="text-[9px] text-white/30 uppercase tracking-widest font-mono mb-1">سعر الدخول المرجح</p>
            ...
            <div className="grid grid-cols-3 divide-x divide-white/[0.05] border-b border-white/[0.05]" dir="rtl">
              {([
                ...
                <div key={t.label} className="py-3 px-2 text-center">
                  <p className="text-[9px] text-white/25 font-medium mb-1 leading-tight">{t.label}</p>
                  <p className="text-xs font-mono font-bold text-white tabular-nums">${fmtPrice(t.price)}</p>
                  <p className="text-[10px] font-mono font-bold text-emerald-400 tabular-nums">
                    +{t.pct.toFixed(2)}%
                  </p>
                </div>
              ))}
            ...
            <div className="mx-4 my-3 rounded-xl border border-white/[0.07] border-t-orange-500 bg-[#0d0d0d] overflow-hidden" dir="rtl">
              ...
              <div className="px-3 py-3">
                <p className="text-[9px] text-white/35 leading-snug mb-2">
                  وقف الخسارة (SL) — الإلغاء الصارم: إغلاق يومي أسفل هذا المستوى.
                </p>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-white/30 font-mono">-{setup.slDropPct.toFixed(2)}%</span>
                  <span className="text-base font-mono font-bold text-orange-400 tabular-nums">${fmtPrice(setup.sl)}</span>
                </div>
              </div>
            </div>

            {/* Risk management box */}
            <div className="mx-4 mb-3 grid grid-cols-2 gap-2" dir="rtl">
              <div className="rounded-xl border border-red-500/20 bg-red-500/[0.05] px-3 py-3 text-right">
                <p className="text-[9px] text-white/30 mb-1">تأكل المحفظة الفعلي</p>
                <p className="text-base font-mono font-bold text-red-400 tabular-nums">-{setup.actualRiskPct.toFixed(2)}%</p>
              </div>
              <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-3 py-3 text-right">
                <p className="text-[9px] text-white/30 mb-1">حجم الدخول المسموح</p>
                <p className="text-base font-mono font-bold text-orange-300 tabular-nums">{setup.positionSizePct.toFixed(1)}%</p>
                <p className="text-[9px] text-white/20 tabular-nums">≈ ${fmtPrice(setup.positionUSDT)} · أقصى 20%</p>
              </div>
            </div>

            {/* Footer disclaimer */}
            <div className="mx-4 mb-4 rounded-xl bg-white/[0.02] border border-white/[0.04] px-3 py-3" dir="rtl">
              <p className="text-[10px] text-white/25 leading-relaxed text-right">
