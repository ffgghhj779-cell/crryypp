...
import { calculateFibMatrix } from '@/lib/algorithms/fibMatrix';
import { computeGann } from '@/lib/algorithms/gann';
import { computeGann144 } from '@/lib/algorithms/gann144';
import { computeSQ9 } from '@/lib/algorithms/sq9';
import { calculateMomentumIntelligence } from '@/lib/algorithms/momentum';
import { generateVIP1Setup } from '@/lib/algorithms/vip1';
import { generateVIP2Setup } from '@/lib/algorithms/vip2';
import { generateVIP3Setup } from '@/lib/algorithms/vip3';
import { calculateVIP } from '@/lib/algorithms/vip';
import { analyzeEmaRibbon } from '@/lib/algorithms/emaRibbon';
import { calculateTrendCompass } from '@/lib/algorithms/trendCompass';
import { analyzeMatrix4x4 } from '@/lib/algorithms/matrix4x4';
import { analyzeTripleLens } from '@/lib/algorithms/tripleLens';
import { calculateUnifiedDecision } from '@/lib/algorithms/unifiedDecision';
import { calculateVolatility } from '@/lib/algorithms/volatility';
import { calculateTrendProbability } from '@/lib/algorithms/trendProbability';
import { scanHarmonicPatterns } from '@/lib/algorithms/harmonicScanner';
import { scanClassicPatterns } from '@/lib/algorithms/patternScanner';
import { analyzeMultiFractal } from '@/lib/algorithms/multiFractal';
import { analyzeMFIMTF } from '@/lib/algorithms/mfiMtf';
import { calculateMTFCScanner } from '@/lib/algorithms/mtfcScanner';
import { calculateTripleAnalysis } from '@/lib/algorithms/tripleAnalysis';
import { calcTDSequential } from '@/lib/algorithms/demark';
  ...
  async function handleScan() {
    ...
    if (!isWidget) {
      try {
        ...
        if (tool.name === 'DeMark TD') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          const td = calcTDSequential(klines);
          // @ts-ignore
          setGenericResult({
            Status: td.td9reached ? 'TD9 REVERSAL (CRITICAL)' : 'ACTIVE SETUP',
            Direction: td.direction.toUpperCase(),
            Setup_Count: td.setupCount + '/9',
            Price_At_Setup_1: td.priceAtSetup1.toFixed(2),
            Current_Price: td.currentPrice.toFixed(2),
          });
          setPhase('done'); return;
        }

        
        if (tool.name === 'Pattern Scanner') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(scanClassicPatterns(klines) || { Status: 'No patterns found' });
          setPhase('done'); return;
        }
        if (tool.name === 'Harmonic Scanner') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(scanHarmonicPatterns(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'ZigZag Engine') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          const dataPoints = klines.map(k => ({ timestamp: k.time * 1000, high: k.high, low: k.low }));
          // @ts-ignore
          setGenericResult(calculateZigZag(dataPoints.map((dp, i) => ({ ...dp, index: i, price: dp.close || dp.high })), 5));
          setPhase('done'); return;
        }
        if (tool.name === 'Fibonacci Matrix') {
          const [k1H, k4H, k1D] = await Promise.all([
            fetchKlines(symbol, '1h', 100), fetchKlines(symbol, '4h', 100), fetchKlines(symbol, '1d', 100)
          ]);
          // @ts-ignore
          setGenericResult(calculateFibMatrix(k1D, k4H, k1H));
          setPhase('done'); return;
        }
        if (tool.name === 'Gann Time Wheel') {
          // @ts-ignore
          setGenericResult(computeGann(new Date()));
          setPhase('done'); return;
        }
        if (tool.name === 'Gann 144 Star') {
          const klines = await fetchKlines(symbol, timeframe, 100);
          // @ts-ignore
          setGenericResult(computeGann144({ symbol: symbol, anchorType: 'bottom', anchorDate: new Date(), anchorPrice: klines[klines.length-1].close }));
          setPhase('done'); return;
        }
        if (tool.name === 'SQ9 - Square of Nine') {
          const klines = await fetchKlines(symbol, timeframe, 100);
          // @ts-ignore
          setGenericResult(computeSQ9({ symbol: symbol, pivot: klines[klines.length-1].close, step: 10, size: 3 }));
          setPhase('done'); return;
        }
        if (tool.name === 'Momentum Intelligence') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          // @ts-ignore
          setGenericResult(calculateMomentumIntelligence(symbol, klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Trading VIP 1') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(generateVIP1Setup(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Trading VIP 2') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(generateVIP2Setup(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Trading VIP 3') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(generateVIP3Setup(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Trading VIP 4' || tool.name === 'Trading VIP 5') {
          const [k15M, k1H, k4H, k1D] = await Promise.all([
            fetchKlines(symbol, '15m', 150), fetchKlines(symbol, '1h', 150), fetchKlines(symbol, '4h', 150), fetchKlines(symbol, '1d', 150)
          ]);
          // @ts-ignore
          setGenericResult(calculateVIP(k15M, k1H, k4H, k1D));
          setPhase('done'); return;
        }
        if (tool.name === 'EMA Ribbon') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(analyzeEmaRibbon(symbol, klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Trend Compass') {
          const klines = await fetchKlines(symbol, timeframe, 200);
          // @ts-ignore
          setGenericResult(calculateTrendCompass(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Matrix 4x4') {
          const klines = await fetchKlines(symbol, timeframe, 100);
          // @ts-ignore
          setGenericResult(analyzeMatrix4x4({ [timeframe]: klines }, klines[klines.length-1].close, symbol));
          setPhase('done'); return;
        }
        if (tool.name === 'Triple Lens') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(analyzeTripleLens(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Unified Decision - 6 Tools') {
          const klines = await fetchKlines(symbol, timeframe, 300);
          // @ts-ignore
          setGenericResult(calculateUnifiedDecision(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'ATR Volatility') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          // @ts-ignore
          setGenericResult(calculateVolatility(klines, 14));
          setPhase('done'); return;
        }
        if (tool.name === 'Trend Probability') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          // @ts-ignore
          setGenericResult(calculateTrendProbability(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'Multi-Scale Fractal' || tool.name === 'Fractal Detector') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          // @ts-ignore
          setGenericResult(analyzeMultiFractal(klines));
          setPhase('done'); return;
        }
        if (tool.name === 'MFI MTF') {
          const [k1H, k4H, k1D] = await Promise.all([
            fetchKlines(symbol, '1h', 150), fetchKlines(symbol, '4h', 150), fetchKlines(symbol, '1d', 150)
          ]);
          // @ts-ignore
          setGenericResult(analyzeMFIMTF(k1H, k4H, k1D));
          setPhase('done'); return;
        }
        if (tool.name === 'MTFC Scanner') {
          const [k15M, k1H, k4H] = await Promise.all([
            fetchKlines(symbol, '15m', 150), fetchKlines(symbol, '1h', 150), fetchKlines(symbol, '4h', 150)
          ]);
          // @ts-ignore
          setGenericResult(calculateMTFCScanner(k15M, k1H, k4H));
          setPhase('done'); return;
        }
        if (tool.name === 'Triple Analysis') {
          const klines = await fetchKlines(symbol, timeframe, 150);
          // @ts-ignore
          setGenericResult(calculateTripleAnalysis(klines));
          setPhase('done'); return;
        }
        
        if (!isWidget && phase === 'scanning') {
           // We'll let it reach the end, but we shouldn't use simulateScan.
        }
