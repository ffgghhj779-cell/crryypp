    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          <span className="text-base text-white/70">Portfolio Drawdown</span>
      ...
      <div className="p-5 rounded-2xl border border-white/[0.05] bg-white/[0.02] flex flex-col items-center gap-1">
        ...
        {requiredGain > 200 && (
          <span className="text-sm text-orange-400/70 mt-1">⚠️ Extreme drawdown — capital preservation is critical</span>
        )}
      </div>
      ...
      <div className="rounded-xl border border-white/[0.05] overflow-hidden">
        ...
        {[[10,11.11],[25,33.33],[50,100],[75,300],[90,900]].map(([dd, rr]) => (
          <div key={dd} className={`grid grid-cols-2 text-base font-mono px-5 py-4.5 border-t border-white/[0.03] ${Math.abs(drawdown - dd) < 5 ? 'bg-orange-500/5 text-white' : 'text-white/50'}`}>
...
function DailyBriefing() {
  ...
  if (error) return (
    <div className="text-center py-8 space-y-2">
      <p className="text-red-400 text-base">Failed to load market data</p>
  ...
  if (!data) return (
    <div className="flex flex-col items-center gap-3 py-10">
      ...
      <p className="text-white/30 text-base">Analyzing market data...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className={`p-6 rounded-2xl border flex items-start gap-3 ${
        ...
        <div>
          <h3 className={`font-bold text-base ${data.isBull ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      <div className="flex justify-between items-center p-6 rounded-xl bg-white/[0.02] border border-white/[0.05]">
        <span className="text-base text-white/60">Current BTC Price</span>
        ...
        {sessions.map(s => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`font-semibold text-base ${s.active ? 'text-orange-400' : 'text-white/50'}`}>{s.name}</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={`text-sm font-semibold ${s.active ? 'text-orange-400' : 'text-white/20'}`}>
...
function EconomicCalendar() {
  ...
  if (!events.length) return (
    <p className="text-center text-white/30 text-base py-8">No upcoming high-impact events.</p>
  );
      ...
      {events.map((item, i) => (
        <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.05]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-base font-medium text-white/90 truncate">{item.title}</p>
            <p className="text-sm text-white/30 font-mono mt-0.5 tabular-nums">{item.date} · {item.time} UTC</p>
          </div>
...
function EmptyState({ icon, title, sub }: { icon: string; title: string; sub: string }) {
  return (
    <div className="flex flex-col items-center gap-3 py-12 text-center" dir="rtl">
      ...
      <p className="text-sm text-white/20 leading-relaxed max-w-[220px]">{sub}</p>
    </div>
  );
}
    ...
    <div className="space-y-2.5">
      {favoriteAssets.length === 0 ? (
        ...
        <div key={asset} className="flex items-center justify-between p-6 rounded-xl bg-white/[0.02] border border-white/[0.05]">
          ...
          <span className="text-sm text-orange-400 font-semibold uppercase tracking-wider">مراقبة</span>
        </div>
      ))}
    </div>
  );
...
function MarketCap() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-16 gap-3">
      ...
      <span className="text-white/30 text-base">جار تحميل بيانات السوق...</span>
    </div>
  );

  if (error) return (
    <p className="text-center text-red-400 text-base py-8">⚠️ {error}</p>
  );
...
function FearGreedDetail() {
  ...
  if (!data) return (
    <div className="flex flex-col items-center justify-center h-full gap-3">
      <div className="w-6 h-6 border-2 border-orange-500/40 border-t-orange-500 rounded-full animate-spin" />
      <p className="text-white/30 text-sm">Fetching sentiment data...</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 gap-6 py-4">
      ...
      <div className="text-center">
        ...
        <p className="text-base font-bold uppercase tracking-widest mt-1" style={{ color }}>{data.label}</p>
      </div>
