      ...
      <div className="px-4 flex flex-col gap-5 mt-4">
        {isLoading || !predicted ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-fuchsia-500/80 font-bold tracking-widest uppercase text-xs animate-pulse">جاري توقع الشمعة القادمة...</p>
          </div>
        ) : (
          <motion.div 
          ...
          >
            <div className="flex justify-center mb-4">
              <h2 className="text-sm font-bold text-white/70 border-b border-white/10 pb-2">
            ...
            <div className="flex items-center justify-center py-8 relative">
              ...
              <div className="relative flex flex-col items-center justify-center w-32 h-48">
                ...
                >
                  <span className="text-[10px] font-black text-white/80 tracking-widest uppercase opacity-70">
                    مخفية
                  </span>
                </motion.div>
              </div>
              ...
              <div className="absolute right-4 top-0 flex items-center gap-2">
                <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest">أعلى سعر متوقع</span>
                <span className="text-sm font-mono font-black text-white/90">{formatPrice(predicted.high)}</span>
              </div>
              
              <div className="absolute left-4 bottom-0 flex items-center gap-2">
                <span className="text-sm font-mono font-black text-white/90">{formatPrice(predicted.low)}</span>
                <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest">أدنى سعر متوقع</span>
              </div>
            </div>

            {/* Data Grid */}
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className={`rounded-xl p-4 flex flex-col gap-1 items-center justify-center border ${predicted.isBullish ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-red-500/5 border-red-500/10'}`}>
                <span className="text-[10px] text-white/50 uppercase font-bold tracking-widest">سعر الإغلاق المتوقع</span>
                <span className={`text-xl font-mono font-black ${predicted.isBullish ? 'text-emerald-400' : 'text-red-400'}`}>
                  {formatPrice(predicted.close)}
                </span>
              </div>
              <div className="rounded-xl p-4 flex flex-col gap-1 items-center justify-center border border-white/5 bg-white/[0.02]">
                <span className="text-[10px] text-white/50 uppercase font-bold tracking-widest">الاتجاه المتوقع</span>
