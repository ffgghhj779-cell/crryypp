      ...
      <div className="px-4 flex flex-col gap-5 mt-4">
        {isLoading || !scanResult ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-purple-500/80 font-bold tracking-widest uppercase text-xs animate-pulse">جاري مسح السوق...</p>
          </div>
        ) : (
          <motion.div 
          ...
          >
            ...
            <div className="rounded-2xl border border-white/[0.05] bg-[#0d0d0d] overflow-hidden">
              <div className="grid grid-cols-2 bg-white/[0.02] border-b border-white/[0.05] p-4">
                <span className="text-xs font-bold text-white/50 uppercase tracking-widest">الإطار الزمني</span>
                <span className="text-xs font-bold text-white/50 uppercase tracking-widest text-left">حالة الاتجاه</span>
              </div>
              
              <div className="grid grid-cols-2 p-4 border-b border-white/[0.02] items-center">
                <span className="text-sm font-bold text-white/80">الاتجاه الطويل (Long-term)</span>
                <span className={`text-sm font-black text-left ${scanResult.direction === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>
                  {scanResult.longTerm}
                </span>
              </div>

              <div className="grid grid-cols-2 p-4 items-center">
                <span className="text-sm font-bold text-white/80">الاتجاه القصير (Short-term)</span>
                <span className={`text-sm font-black text-left ${scanResult.direction === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            {scanResult.isAligned ? (
              <motion.div 
                ...
                className={`rounded-3xl border p-6 flex items-start gap-4 shadow-2xl relative overflow-hidden ${scanResult.direction === 'buy' ? 'bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_40px_rgba(16,185,129,0.2)]' : 'bg-red-500/10 border-red-500/50 shadow-[0_0_40px_rgba(239,68,68,0.2)]'}`}
              >
