'use client';

import { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { TriangleRight, ScanSearch, AlertCircle } from 'lucide-react';
import { ToolPageHeader } from '@/components/tools/ToolPageHeader';
import { slugToTool } from '@/lib/tools/registry';
import { fetchKlines, Kline } from '@/lib/binance/fetcher';
import { SymbolDropdown } from '@/components/tools/SymbolDropdown';
import { notFound } from 'next/navigation';
import { ToolChart } from '@/components/tools/ToolChart';
import { detectWedge, PatternResult } from '@/lib/algorithms/patterns';

export default function WedgeScannerPage() {
  const [symbol, setSymbol] = useState('BTCUSDT');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [result, setResult] = useState<PatternResult | null>(null);
  const [klines, setKlines] = useState<Kline[]>([]);

  
  const handleScan = useCallback(async () => {
    setError(''); setLoading(true); setResult(null);
    try {
      const fetchedKlines = await fetchKlines(symbol.toUpperCase().trim(), '4h', 300);
      if (fetchedKlines.length < 60) throw new Error('بيانات غير كافية للتحليل (نحتاج 60 شمعة على الأقل)');
      
      setKlines(fetchedKlines);
      const res = detectWedge(fetchedKlines);
      setResult(res);
    } catch (e: any) { 
      setError(e.message || 'حدث خطأ غير متوقع');
    } finally { 
      setLoading(false); 
    }
  }, [symbol]);

  // Generate trendline data for ToolChart if pattern is detected
  const upperLineData = [];
  const lowerLineData = [];

  if (result?.detected && result.swingHighs && result.swingLows && result.upperSlope !== undefined && result.lowerSlope !== undefined) {
    const firstHighIdx = result.swingHighs[0].index;
    const firstLowIdx = result.swingLows[0].index;
    const firstHighPrice = result.swingHighs[0].price;
    const firstLowPrice = result.swingLows[0].price;

    const startIdx = Math.min(firstHighIdx, firstLowIdx);
    const endIdx = klines.length - 1 + (result.apexEstimate ? Math.min(result.apexEstimate, 20) : 0); // Extend slightly into future

    for (let i = startIdx; i <= endIdx; i++) {
      // Use time of existing klines, or extrapolate future time based on 4h intervals (14400000 ms)
      const time = i < klines.length ? klines[i].time : klines[klines.length - 1].time + (i - klines.length + 1) * 14400000;
      
      upperLineData.push({
        time,
        value: firstHighPrice + result.upperSlope * (i - firstHighIdx)
      });

      lowerLineData.push({
        time,
        value: firstLowPrice + result.lowerSlope * (i - firstLowIdx)
      });
    }
  }

  const isBullish = result?.type === 'FALLING'; // Falling wedges are bullish breakouts
  const isBearish = result?.type === 'RISING'; // Rising wedges are bearish breakdowns
  const colorHex = isBullish ? '#10b981' : isBearish ? '#ef4444' : '#6b7280';

  const tool = slugToTool('wedge-scanner');
  if (!tool) return notFound();

  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] overflow-y-auto pb-10" dir="rtl">
      <ToolPageHeader tool={tool} />
      
      <div className="px-5 pt-5 pb-4 flex flex-col gap-1">
        <span className="text-sm font-black text-violet-400/70 tracking-widest uppercase border border-violet-500/20 bg-violet-500/10 px-2.5 py-1 rounded-full w-fit flex items-center gap-1">
          <TriangleRight className="w-3 h-3" /> Wedge Scanner
        </span>
        <h1 className="text-xl font-black text-white mt-1">مكتشف نماذج الأوتاد (Wedges)</h1>
        <p className="text-sm text-white/40 font-mono">Rising & Falling Wedge Detector — يحلل الانعكاس السعري</p>
      </div>

      <div className="px-5 flex flex-col gap-5">
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 flex flex-col gap-4">
          <SymbolDropdown value={symbol} onChange={setSymbol} />
          
          <AnimatePresence>
            {error && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex items-center gap-2 rounded-xl bg-red-500/10 border border-red-500/20 px-3 py-3">
                <AlertCircle className="w-4 h-4 text-red-400 shrink-0" />
                <p className="text-sm text-red-300">{error}</p>
              </motion.div>
            )}
          </AnimatePresence>

          <button onClick={handleScan} disabled={loading} className="w-full flex items-center justify-center gap-3 rounded-xl py-4 font-black text-base text-white disabled:opacity-50 transition-all"
            style={{ background: loading ? '#1a1a1a' : 'linear-gradient(135deg,#8b5cf6,#6d28d9)', boxShadow: !loading ? '0 0 20px rgba(139,92,246,0.25)' : 'none' }}>
            {loading ? <span className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <ScanSearch className="w-6 h-6" />}
            {loading ? 'جاري الفحص (4H)...' : 'بحث عن نموذج الوتد'}
          </button>
        </div>

        <AnimatePresence>
          {result && (
            <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} exit={{opacity:0}} className="flex flex-col gap-4">
              
              {!result.detected ? (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center flex flex-col items-center gap-3">
                  <ScanSearch className="w-12 h-12 text-white/20" />
                  <p className="text-lg font-bold text-white/60">لم يتم رصد نموذج وتد واضح</p>
                  <p className="text-sm text-white/40">السوق حالياً إما يتحرك بشكل عرضي أو في اتجاه واضح بدون انحسار سعري.</p>
                </div>
              ) : (
                <>
                  <div className="rounded-2xl bg-[#050505] p-4 border border-violet-500/20 shadow-lg shadow-violet-500/5">
                    <div className="flex justify-between items-center mb-4">
                      <p className="text-sm font-bold text-violet-400 uppercase tracking-widest flex items-center gap-2">
                        <TriangleRight className="w-4 h-4" /> Wedge View
                      </p>
                      <p className="text-xs font-mono text-white/40">{symbol.toUpperCase()} • 4H</p>
                    </div>
                    <ToolChart 
                      klines={klines}
                      height={300}
                      overlays={[
                        { type: 'line', data: upperLineData, color: '#ef4444', title: 'Resistance', lineWidth: 2 },
                        { type: 'line', data: lowerLineData, color: '#10b981', title: 'Support', lineWidth: 2 }
                      ]}
                    />
                  </div>

                  <div className="rounded-2xl border p-6 flex items-center justify-between" style={{ borderColor: colorHex + '40', background: colorHex + '0a' }}>
                    <div>
                      <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: colorHex }}>نوع النموذج المكتشف</p>
                      <p className="text-2xl font-black text-white">{result.type === 'RISING' ? 'وتد صاعد (سلبي)' : 'وتد هابط (إيجابي)'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-white/40 mb-1">نسبة الثقة</p>
                      <p className="text-xl font-black font-mono" style={{ color: colorHex }}>{result.confidence}%</p>
                    </div>
                  </div>

                  <div className="rounded-2xl border p-5" style={{ borderColor: colorHex + '30', background: colorHex + '08' }}>
                    <p className="text-sm font-black uppercase tracking-widest mb-2" style={{ color: colorHex }}>التحليل والتوقع</p>
                    <p className="text-sm text-white/70 leading-relaxed">
                      {isBullish 
                        ? 'تم رصد وتد هابط (Falling Wedge)، وهو نموذج استمراري وانعكاسي إيجابي. يشير إلى انحسار ضغط البيع. توقع اختراقاً لأعلى للمقاومة.'
                        : 'تم رصد وتد صاعد (Rising Wedge)، وهو نموذج سلبي يشير إلى انحسار الزخم الشرائي وضعف المشتري. توقع كسراً لأسفل لخط الدعم.'}
                      {result.apexEstimate && result.apexEstimate <= 5 && ' الكسر أو الاختراق متوقع حدوثه قريباً جداً (من 1 إلى 5 شموع).'}
                    </p>
                  </div>
                </>
              )}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
    </div>
  );
}
